...
/**
 * 使用 Kimi 提取图片文字（file-extract）
 */
async function kimiExtract(filePath, kimiClient) {
...
/**
 * 使用 DeepSeek 清洗并格式化文字
 */
async function deepseekClean(rawText, deepseekClient) {
  ...
  return formatCompletion.choices[0]?.message?.content || rawText;
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 3. 主路径：先用 Kimi OCR 提取文字
    try {
      extractedText = await kimiExtract(filePath, kimiClient);
    } catch (kimiErr) {
      console.warn('Kimi OCR 失败，准备降级到百度 OCR:', kimiErr.message);
      // 降级到百度 OCR，如果也失败则直接抛出外层 catch
      extractedText = await baiduOcr(filePath);
    }

    // 4. 如果 Kimi 返回空，也尝试百度 OCR 降级
    if (!extractedText || !extractedText.trim()) {
      console.warn('Kimi OCR 返回空文字，降级到百度 OCR');
      extractedText = await baiduOcr(filePath);
    }
    ...
    // 5. 直接调用 DeepSeek 清洗（去掉原来 Kimi 的二次整理，节省 3-8 秒）
    const formattedText = await deepseekClean(extractedText, deepseekClient);
